...
目前内置画像 `cn_national_joint_2020`（第七次全国人口普查 + IPF 联合分布）覆盖 demographic（年龄×性别×学历×婚姻×职业 真联合 + 省份/家庭规模弱耦合边际）；personality / media 仍为合成占位，需从下列来源补充。早期的边际版 `cn_census_2020`（各维度独立抽样，会产生"20岁已婚"等失真组合）已移除。
...
| 来源 | 补充维度 | 下载地址 |
|------|---------|---------|
| 第七次全国人口普查主要数据 | 性别/年龄/城乡/学历/民族/省份（IPF 联合后入库 `cn_national_joint_2020`） | https://www.stats.gov.cn/sj/pcsj/rkpc/d7c/ |
...
1. 下载报告（PDF/Excel）。图表型 PDF 可拆成图片，由模型读图提取数字（参见 `tools/synthpop/` 七普表的提取方式）。
...
- **算法：IPU（分层 IPF）** —— 用省级交叉表做「个人层」约束 + 家庭数据做「家庭层」约束，比纯 IPF 多利用一层（虚拟人自身属性真实 + 其家庭成员组合也真实）。零额外数据成本、合规。将来用**自有问卷**当种子升级为「IPU + 种子」。
- **不用 CFPS/CGSS 等学术微观数据**：虽含真实个体（理想种子），但**协议禁止商用**，万象是商业产品，违约有法律/声誉/尽调风险，且与「数据可信合规」卖点冲突。技术上虽难被发现，但不值得赌。
- **全维度联合真数据公开渠道拿不到**：统计/普查只发布到两两/三三交叉表，微观个体记录涉隐私不公开。行业（含 Aaru）通用做法即「交叉表 + IPF/IPU 推断联合」，本质是高级合成，非真数据。
...
|------|-----------|------|
| IPF | 否 | 基准，只靠交叉表即可 |
| **IPU / 分层 IPF** | 否 | ✅ **本项目选用**——有家庭数据，可同时拟合个人层+家庭层 |
| 最大熵 | 否 | 与 IPF 同档，无明显增益 |
| 贝叶斯网络 | 否（可只用交叉表） | 可解释，但维度多时结构难学稳 |
| 组合优化（模拟退火） | **是** | 有种子后才用 |
| 深度生成（CTGAN/VAE/扩散） | **是（大量微观）** | 黑箱、人口模拟不推荐 |

> 结论：**算法不是瓶颈，数据是**。无种子时 IPF/IPU/最大熵即天花板（信息论上变不出约束里没有的相关性）。针对本项目「有家庭数据、无个体种子」的特点，IPU 优于纯 IPF；有了自有问卷种子后真实度才能再上台阶。

### 数据状态（已基本齐全，公开渠道接近天花板）

已收集在 `docs/persons/`（已加入 .gitignore，不上传 GitHub）：

| 目录 | 内容 | 覆盖维度 | 状态 |
|------|------|---------|------|
| person_pucha | 七普主要数据（IPF 联合后入库 `cn_national_joint_2020`） | 性别/年龄/城乡/学历/民族/省份 | ✅ |
| jiuye | 省份×性别×{年龄/学历/职业/行业} | 职业、行业 | ✅ |
| hunyin | 省份×性别×婚姻、年龄×学历×婚姻 | 婚姻 | ✅ |
| jiating | 家庭户类别（户规模×几代户） | 家庭规模/结构（IPU 家庭层） | ✅ |
| zhufang | 各地区按住房状况分家庭户 | 住房 | ✅ |
| shengyu | 分年龄/孩次生育 | 生育/子女数 | ✅ |
| jiuye_wages | 就业基本情况 + 工资趋势 | 工资 | ✅ |
| shouru | 31 省人均可支配收入 | 收入（分省） | ✅ |
| xiaofei | 居民消费支出结构（八大类） | 消费（场景背景/校准用） | ✅ |
| hulianwang | CNNIC 数字消费报告（112 张） | media 媒介/平台 | ✅ |

**还能再提升精度的，只剩两类（均非必需）**：
- **自有问卷 / CRM / 业务数据**（⭐ 最高价值，真实个体种子，可商用）—— 未来升级路径。
- 细分平台对比（抖音 vs 小红书 vs 微信渗透率）—— 需 QuestMobile / 巨量算数，hulianwang 只给整体渗透。

> 公开统计数据已到天花板：再加交叉表收益递减，省级深度交叉（如「省份×收入×职业」）统计局基本不公开。下一档提升只能靠自有问卷种子。

### 实施计划

1. **第一步（小成本，~¥20）**：用现有 jiuye+hunyin（+家庭数据）跑一版 IPU，生成合成人口样本，用真值表验证质量 → 看效果再决定是否接引擎。
2. **第二步（~¥150）**：效果满意后改造 `PersonaBuilder`（当前 `wanxiang/personas/builder.py` 是逐维度独立 `rng.choices`）支持联合分布/分层抽样 + 画像格式 + 上传校验 + 前端 + 验证。
3. **将来**：自己做问卷调研（几百份即可，问卷星等）→ 自有、可商用、可公开来源的真实种子 → IPU 重加权升级。

> 现状：内置画像 `cn_national_joint_2020`（七普 + 全维 IPF 联合分布：年龄×性别×学历×婚姻×职业 真联合 + 省份/家庭规模弱耦合边际，留出重建 TVD 0.0041 / 误差降低 98.5%）；收入/家庭成员等维度数据已备齐但尚未入库；personality 仍为合成占位。早期边际版 `cn_census_2020`（独立抽样）已移除。
